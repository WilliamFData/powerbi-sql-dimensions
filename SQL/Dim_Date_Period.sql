...
WITH CTEDATES AS (
    SELECT
        ...
        DATEADD('DAY', 6, DATE_TRUNC('WEEK', DATEADD('WEEK', -1, TODAY)))
            AS END_OF_PRIOR_WEEK,
        DATE_TRUNC(MONTH, YESTERDAY) AS START_OF_MONTH,
        DATE_TRUNC(MONTH, DATEADD(MONTH, -1, YESTERDAY))
            AS START_OF_PRIOR_MONTH,
        LAST_DAY(DATEADD(MONTH, -1, YESTERDAY)) AS END_OF_PRIOR_MONTH,
        CASE
            WHEN MONTH(YESTERDAY) >= 7 THEN TO_DATE(YEAR(YESTERDAY) || '-07-01')
            ELSE TO_DATE((YEAR(YESTERDAY) - 1) || '-07-01')
        END AS START_OF_FIN_YEAR,
        CASE
            WHEN MONTH(YESTERDAY) < 7 THEN TO_DATE(YEAR(YESTERDAY) || '-06-30')
...
WHERE
    1 = 1
    AND DATE BETWEEN (SELECT START_OF_PRIOR_WEEK FROM CTEDATES
...
WHERE
    1 = 1
    AND DATE BETWEEN (SELECT START_OF_MONTH FROM CTEDATES
...
WHERE
    1 = 1
    AND DATE BETWEEN (SELECT START_OF_PRIOR_MONTH FROM CTEDATES
...
WHERE
    1 = 1
    AND DATE BETWEEN (SELECT START_OF_FIN_YEAR FROM CTEDATES
...
WHERE
    1 = 1
    AND DATE BETWEEN (SELECT START_OF_PRIOR_FY FROM CTEDATES
...
WHERE
    1 = 1
    AND DATE BETWEEN (SELECT START_OF_YEAR FROM CTEDATES
    ) AND (SELECT YESTERDAY FROM CTEDATES
    )
UNION ALL
-- ====================================================
